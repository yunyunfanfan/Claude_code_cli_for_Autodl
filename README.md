...
## 第二步：填入你的 API Key和中转站地址

用任意编辑器打开脚本，将 `ANTHROPIC_AUTH_TOKEN` 的值替换为你自己的Key：
...
| 写入 settings.json | 注入 Key 和中转地址 | `ANTHROPIC_BASE_URL` 指向 中转站，`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` 禁用遥测等会访问境外地址的后台请求 |
